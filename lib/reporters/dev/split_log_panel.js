...
function failureDisplay(item) {
  ...
  if (stacktrace) {
    ...
    var sourceMapTransformer = createSourceMapTransformer({
      newFileRegex: /^(\s*)?(?:.*?@)?(\D[^@]*?):(\d+):\d+/,
      newFilePath: function(match) {
        return match[2];
      }
//      defaultPrevFileRegex: /^(\s*)(?:.*?@)?(.*?):(\d*)/
    });
    // return require('string-to-stream')(stacktrace).pipe(sourceMapTransformer).pipe(process.stdout);
    return new Bluebird.Promise(function(resolve) {
      require('string-to-stream')(stacktrace).pipe(sourceMapTransformer).pipe(
        ps.reduce(function(acc, smt) {
          return acc + smt;
        })
      ).promise().then(function(convertedStacktrace) {
        resolve(finish(convertedStacktrace));
      });
    });
  } else {
    if (item.file) {
...
function failedTestDisplay(test) {
  ...
  return Bluebird.Promise.map(failedItems, function(failedItem) {
    return failureDisplay(failedItem);
...
module.exports = View.extend({
  ...
  initialize: function() {
    ...
    this.asyncResultsDisplay({silent: true}, function() {
      self.syncMessages({silent: true});
      self.render();
    });
  },
  ...
  asyncResultsDisplay: function(options) {
    var self = this;
    this.getResultsDisplayText(function(displayText) {
      self.topPanel.set('text', displayText, options);
    });
  },
  ...
  getResultsDisplayText: function(cb) {
    var runner = this.get('runner');
    var results = runner.get('results');
    ...
    var pendingOut = '';
    function finish() {
      cb(styledString(out, {foreground: 'cyan'})
    ...
    if (results && (tests = results.get('tests'))) {
      ...
      if (!total) {
        ...
        if (failedTests.length > 0) {
          Bluebird.Promise.map(failedTests, function(failedTest) {
            return failedTestDisplay(failedTest);
          }).then(function(mapped) {
            out += mapped.join('\n');
            finish();
          });
